...
const HeartbeatMonitor: React.FC<HeartbeatMonitorProps> = ({ speed, isAlert }) => {
  ...
  return (
    <div className="w-full h-24 md:h-32 lg:h-48 relative overflow-hidden flex items-center justify-center">
      <svg 
        width="100%" 
        height="100%" 
        viewBox="0 0 800 200" 
...
const Act0BiometricAnalysis: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-between p-4 md:p-6 sm:p-12 font-mono relative overflow-y-auto landscape:justify-center landscape:gap-4 transition-colors duration-500 
      ...
      <div className="w-full flex flex-col items-center text-center space-y-1 text-zinc-600 uppercase tracking-widest z-30 pt-2 landscape:hidden">
      ...
      <div className="flex-1 w-full max-w-4xl flex flex-col items-center justify-center space-y-6 md:space-y-12 z-10 landscape:space-y-4">
        <h1 className={`text-red-500 text-2xl sm:text-5xl md:text-6xl font-black text-center uppercase px-4 tracking-tighter leading-[0.9] transition-all duration-300 
          ${isAlert ? 'scale-110 drop-shadow-[0_0_20px_rgba(239,68,68,0.8)]' : ''}`}>
          {isAlert ? "SISTEMA_SOBRECARREGADO" : "LENDO BIOMETRIA"}
        </h1>

        <div className="w-full relative py-2 md:py-10 landscape:py-0">
          <HeartbeatMonitor speed={displayedProgress} isAlert={isAlert} />
          <div className={`absolute top-0 right-0 sm:right-10 font-bold text-3xl sm:text-7xl italic transition-colors duration-200 ${isAlert ? 'text-red-500' : 'text-red-900/40'}`}>
            {Math.floor(72 + (displayedProgress * 1.15) + (Math.random() * 5))} <span className="text-sm sm:text-2xl not-italic opacity-50">BPM</span>
          </div>
        </div>

        <div className="min-h-[60px] md:min-h-[100px] flex items-center justify-center px-4 w-full landscape:min-h-[40px]">
          <p className={`text-lg sm:text-3xl font-mono text-center transition-all duration-200 uppercase tracking-tight 
            ${isAlert 
      ...
      <div className="w-full max-w-4xl space-y-4 pb-4 z-30 landscape:pb-2">
        <div className="flex justify-between items-end mb-1">
          <div className={`text-[8px] md:text-[10px] uppercase tracking-widest font-bold transition-colors ${isAlert ? 'text-red-500' : 'text-zinc-700'}`}>
            Status Biométrico
          </div>
          <div className={`text-2xl md:text-4xl font-black italic transition-all ${isAlert ? 'text-red-500' : 'text-red-400'}`}>
            {Math.floor(displayedProgress)}%
          </div>
        </div>

        <div className={`w-full bg-zinc-900/50 h-4 md:h-6 rounded-full border-2 transition-colors duration-500 overflow-hidden relative 
          ${isAlert ? 'border-red-600' : 'border-red-900/30'}`}>
          <div className="h-full bg-gradient-to-r from-red-950 via-red-600 to-red-400 transition-all duration-100 ease-out" 
               style={{ width: `${displayedProgress}%` }} />
        </div>
        
        <div className="text-center text-[8px] md:text-[10px] text-zinc-800 uppercase tracking-[0.4em] opacity-40 landscape:hidden">
          COLETA_DADOS_BIOMETRICOS
        </div>
      </div>
    </div>
  );
